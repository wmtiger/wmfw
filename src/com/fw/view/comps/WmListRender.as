package com.fw.view.comps
{
	 ...
	 */
	public class WmListRender extends FeathersControl implements IListItemRenderer
	{
		public function WmListRender(handler:IRenderClickHandler = null)
		{
			_handler = handler;
			stateToValue[STATE_UP] = new Quad(1, 1, 0xE0FFFF);
			stateToValue[STATE_UP].alpha = 0;
			//stateToValue[STATE_DOWN] = new Quad(1, 1, 0xE0FFFF);
			//stateToValue[STATE_HOVER] = new Quad(1, 1, 0xFFD39B);
			//stateToValue[STATE_DISABLED] = new Quad(1, 1, 0xDCDCDC);
			//stateToSelectedValue[STATE_UP] = new Quad(1, 1, 0xff0000);
			//stateToSelectedValue[STATE_DOWN] = new Quad(1, 1, 0xff0000);
			//stateToSelectedValue[STATE_HOVER] = new Quad(1, 1, 0xCD5C5C);
			//stateToSelectedValue[STATE_DISABLED] = new Quad(1, 1, 0xDCDCDC);
//			stateToSelectedValue[STATE_UP] = new Quad(1, 1, 0xE0FFFF);
//			stateToSelectedValue[STATE_UP].alpha = 0.2;
		 ...
		 * 是否可以从选种状态变回非选中状态
		 */
		protected var unSelectable:Boolean = true;
		...
		public function set owner(value:List):void
		{
			...
			if (this._owner)
			{
				...
				List(this._owner).removeEventListener(FeathersEventType.SCROLL_COMPLETE,scrollCompleteHandler);
			}
			this._owner = value;
			if (this._owner)
			{
				...
				list.addEventListener(FeathersEventType.SCROLL_COMPLETE,scrollCompleteHandler);
			}
			this.invalidate(INVALIDATION_FLAG_DATA);
		}
...
//		
//		protected var _backgroundFactory:Function = defaultBackgroundFactory;
//		
...
//			if(this._backgroundFactory == value){
//				return;
//			}
//			
//			this._backgroundFactory = value;
...
//		}
		...
		protected function scrollCompleteHandler(event:Event):void
		{
		}
...
//		protected function defaultBackgroundFactory():DisplayObject{
//			return new Quad(this.actualWidth,this.actualHeight,0xff0000);
//		}
		...
		override protected function draw():void
		{
			...
			if (styleInvalid)// || stateInvalid || selectionInvalid 滑过不需要改变样式，先注释掉 2013-06-14
			{
				this.refreshStyle();
			}
...
//			image.source = "/assets/images/aeon/button-disabled-skin.png";
//			image.source = "metalworks.png";
//			addChild(image);
		...
		protected function refreshSkin():void
		{
			...
			if (this.currentSkin != oldSkin)
			{
				...
				if (this.currentSkin)
				{
					if (this.isSelected) {
						// TODO, 背景以上
						this.addChildAt(this.currentSkin, _background==null?0:1);
					} else {
		...
		public function set normalSkin(obj:DisplayObject):void {
			stateToValue[STATE_UP] = obj;
		}
		/**
		 * @private
			...
			if (this._touchPointID >= 0)
			{
				...
				else if (touch.phase == TouchPhase.ENDED)
				{
					...
					if (isInBounds)
					{
						...
						if (unSelectable || this._isSelected == false) {
